...
#define COMANDO_SACO_AZUL    "sacoAzul"
...
void *thread_main(void *arg_ptr) {
  ...
  while(1) {
	...
	else if(cmd.operacao == OP_SACO_AZUL)
    {
      if (transferirClandestino (cmd.idConta) < 0)
        printf("Erro no Saco Azul");
      else
        printf("%s: OK\n", COMANDO_SACO_AZUL);
    }
...
int main (int argc, char** argv) {
  ...
  */
  ...
  while (1) {
    ...
    /*Saco Azul*/
    else if (strcmp(args[0], COMANDO_SACO_AZUL) == 0) {
      if (numargs < 2) {
        printf("%s: Sintaxe inválida, tente de novo.\n", COMANDO_SACO_AZUL);
        continue;
      }
      cmd.operacao = OP_SACO_AZUL;
      cmd.idConta = atoi(args[1]);
      cmd.idContaDestino = 1;
